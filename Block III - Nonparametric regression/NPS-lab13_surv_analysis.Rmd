...
As usual it is edifying to try and hard code the quantities
...
does so for the quantities returned by `survfit`: we will not comment it, but it could be useful for you to go
...
older than 70 is significant. Again, with a little bit of data wrangling we can manually compute the required quantities for the log-rank test statistics:
